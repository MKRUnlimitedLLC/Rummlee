...
/** Shrink a phone photo to a JPEG. Canvas output drops location and other camera data. */
export async function compressPhoto(file: File): Promise<string> {
  ...
  if (/heic|heif/i.test(file.type) || /\.heic$|\.heif$/i.test(file.name)) {
    throw new Error("Save that phone photo as a JPEG, then choose it again.");
  }
  ...
  try {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      last = encodeJpeg(bitmap, edge, quality);
...
function encodeJpeg(bitmap: ImageBitmap, maxEdge: number, quality: number) {
