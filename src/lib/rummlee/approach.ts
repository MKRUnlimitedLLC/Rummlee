...
export async function ensureApproach(sql: Awaited<ReturnType<typeof getSql>>) {
  await sql`
  ...
  `;
  await sql`alter table approach_pings add column if not exists phase text not null default 'close'`;
}
